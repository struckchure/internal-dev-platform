...
func (m *MachineDao) CreateMachine(args types.CreateMachineArgs) (*db.MachineModel, error) {
	return m.client.Machine.
		CreateOne(
			...
			db.Machine.CPU.Set(args.CPU),
			db.Machine.Memory.Set(args.Memory),
...
func (m *MachineDao) UpdateMachine(args types.UpdateMachineArgs) (*db.MachineModel, error) {
	return m.client.Machine.
		...
		Update(
			...
			db.Machine.CPU.SetIfPresent(args.CPU),
			db.Machine.Memory.SetIfPresent(args.Memory),
...
func NewMachineDao(connection *internals.DatabaseConnection) IMachineDao {
	return &MachineDao{
